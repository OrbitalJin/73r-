...
class mv(Command):
    ...
    def execute(self, args: Optional[dict], options: Optional[dict]) -> None:
        ...
        if not args: return self.sys.display.warning("No source or destination specified. Use -h for help.")
        if len(args) < 2: return self.sys.display.warning("No destination specified.")
        src_, dest_ = args.get(0), args.get(1)

        src = self.sys.disk.current.find(name = src_)
        if not src: return self.sys.display.error(f"File or folder not found: {src_}")
        
        if dest_ == "..": dest = self.sys.disk.current.parent
        else: dest = self.sys.disk.current.find(name = dest_)
        
        if not dest: return self.sys.display.error(f"Destination not found: {dest_}")
        if not isinstance(dest, Folder | DotFolder): return self.sys.display.error(f"Destination is not a folder: {dest_}")
